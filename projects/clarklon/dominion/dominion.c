...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      return adventurerCard(z, drawntreasure, state, currentPlayer, temphand);
			
    case council_room:
      return council_roomCard(currentPlayer, state, handPos);
			
    case feast:
      return feastCard(state, currentPlayer, temphand, choice1);
...
//refactored Smithy card from cardEffect to its own function
//BUG: Reverse symbol, which would make it so the player never draws cards
int smithyCard(int currentPlayer, struct gameState *state, int handPos)
{
  int i;

  //+3 Cards
  //BUG: Will imediately evaluate to false and not run at all, giving user no cards
  for (i = 0; i > 3; i++)
  {
    drawCard(currentPlayer, state);
  }
...
//refactored Adventurer card from cardEffect to its own function
////BUG: Early reshuffule called by calling function when deck size is equal to 1
int adventurerCard(int z, int drawntreasure, struct gameState *state, int currentPlayer, int temphand[])
{
  while(drawntreasure<2){
    //BUG: Should be only '< 1' to prompt reshuffle. Here the inclusion of the additional '=' symbol will cause early reshuffle.
...
//refactored Council Room card from cardEffect to its own function
//BUG: Decrease the number of buys, by inverting the symbols 
int council_roomCard(int currentPlayer, struct gameState *state, int handPos)
{
  ...
  //+1 Buy
  //BUG: Instead of +1 Buy, becomes -1 Buy
  state->numBuys--;
...
//refactored Feast card from cardEffect to its own function
//BUG: changed argument of gainCard method used to add the card to your hand instead of discard (where it should go).
int feastCard(struct gameState *state, int currentPlayer, int temphand[], int choice1)
{
  ...
  while( x == 1) {//Buy one card
    if (supplyCount(choice1, state) <= 0){
      ...
      //BUG: by changing '0' to '2' in the gainCard method, we add the gained card to our hand, when we should add to discard
