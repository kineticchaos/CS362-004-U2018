...
int main()
{
	...
	for (k = 0; k < TEST_RUNS; k++)
	{
		...
		if (cardEffect(council_room, choice1, choice2, choice3, &postTest, handPos, &bonus) == 0)
		{
			...
			else if(!(postTest.handCount[currentPlayer] == (preTest.handCount[currentPlayer] + (preTest.deckCount[currentPlayer] + preTest.discardCount[currentPlayer]))))
			{
				printf("\tTest 1 Failed - Hand Size Incorrect After Card Being Played\n");
                                test1Fails++;
                                passFlag = 0;
			...
			if(preTest.deckCount[currentPlayer] > 3)
			{
			...
			else if((preTest.deckCount[currentPlayer] + preTest.discardCount[currentPlayer]) > 3)
			{
				if (!(postTest.deckCount[currentPlayer] == ((preTest.deckCount[currentPlayer] - 4) + preTest.discardCount[currentPlayer])))
                                {
                                        printf("\tTest 3 Failed - Deck Size Incorrect After Card Being Played\n");
                                        test3Fails++;
                                        passFlag = 0;
			...
			{
                                if (!(postTest.deckCount[currentPlayer] == 0) && (postTest.discardCount[currentPlayer] == 0))
                                {
                                        printf("\tTest 3 Failed - Deck Size Incorrect After Card Being Played\n");
                                        test3Fails++;
                                        passFlag = 0;
			...
			if(preTest.deckCount[nextPlayer] > 0)
			{
			...
			{
				if((preTest.deckCount[nextPlayer] + preTest.discardCount[nextPlayer]) > 0)
				{	 
					if (!((preTest.handCount[nextPlayer] + 1) == postTest.handCount[nextPlayer] && ((preTest.deckCount[nextPlayer] + preTest.discardCount[nextPlayer]) - 1) == postTest.deckCount[nextPlayer]))
                                	{
                                	        printf("\tTest 5 Failed - Opponent Wasn't Able to Draw a Card Properly\n");
                                	        test5Fails++;
                                	        passFlag = 0;
					...
					if (!(postTest.discardCount[nextPlayer] == 0))
                                	{
                                	        printf("\tTest 6 Failed - Opponent's Discard Changed in Unexpected Manner\n");
                                	        test6Fails++;
                                	        passFlag = 0;
	...
	printf("Test 8 - Card is Able to Return Successfully, # of Fails: %d\n", testAllFails);
